...
/*
 * Here we can see that the exportPublicKey function acts as a static / standalone / singleton
 ...
 */
uint32_t exportPublicKey(uint8_t *key, uint32_t *key_size)
{
	...
	/* Invoke command */
	ret = TEEC_InvokeCommand(&session, CMD_GET_PUBLIC_KEY, &operation, &retOrigin);
...
uint32_t init(uint8_t *root_key, uint32_t size)
{
	TEEC_Operation operation = {0};
	TEEC_Result ret;
...
uint32_t doCrypto(uint8_t *key_chain, uint32_t key_count, uint32_t key_len, uint8_t op_cmd,
		  uint8_t *src, uint32_t src_len, uint8_t *dest, uint32_t *dest_len)
{
	...
	/* all operations except OM_OP_CREATE_ROOT_DIRECTORY, utilize the key chain hirarchy */
	if (op_cmd != OM_OP_CREATE_ROOT_DIRECTORY) {
